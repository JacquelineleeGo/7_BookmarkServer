...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            #    Delete the following line -- raise NotImplementedError("Step 3 isn't written yet!")
            self.send_response(404)
            self.send_header('Content-type', 'text/html')
            self.end_headers()
            self.wfile.write("Please provide a longuri or a shortname or both")
        ...
        if CheckURI(longuri):
            ...
            # Delete the following line -- raise NotImplementedError("Step 4 isn't written yet!")
            self.send_response(303)
            ...
            # Delete the following line -- raise NotImplementedError("Step 5 isn't written yet!")
            self.send_response('404')
            self.send_header('Content-type', 'text/html')
            self.end_headers()
            self.wfile('Fetching the long URI failed')
